...
object TwitterHyperLogLog extends Log {
  ...
  def performanceTest(): Unit = {
    val array1 = RandomUtil.randomIntArray(10000000)
    val array2 = RandomUtil.randomIntArray(10000000)

    val hll1 = intArray2Hll(array1)
    val hll2 = intArray2Hll(array2)
    log.info("sizeOfHll1 -> {}", RamUsageEstimator.humanSizeOf(hll1))
    log.info("sizeOfHll2 -> {}", RamUsageEstimator.humanSizeOf(hll2))

    log.info("start")
    val start = System.currentTimeMillis()
    1 to 1000 foreach (_ => andCardinality(Seq(hll1, hll2)))
